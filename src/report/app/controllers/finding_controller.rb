...
class FindingController < ApplicationController
  ...
  def index
    defect_query = "select transactions.uri, location, evidence, defectType.description, finding.responseId as responseId from defect inner join finding on finding.id = defect.findingId inner join defectType on defect.type = defectType.id inner join transactions on transactions.id = finding.responseId order by defectType.type"
